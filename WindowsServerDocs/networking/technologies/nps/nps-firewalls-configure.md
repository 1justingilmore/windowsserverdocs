...
- Microsoft Defender Firewall with Advanced Security on the local server running Network Policy Server (NPS).
- Firewalls running on other computers or hardware devices.

## Windows Firewall on the local NPS

By default, NPS sends and receives RADIUS traffic by using User Datagram Protocol \(UDP\) ports 1812, 1813, 1645, and 1646. Microsoft Defender Firewall on the NPS is automatically configured with exceptions, during the installation of NPS, to allow this RADIUS traffic to be sent and received.

Therefore, if you are using the default UDP ports, you do not need to change the Microsoft Defender Firewall configuration to allow RADIUS traffic to and from NPSs.
